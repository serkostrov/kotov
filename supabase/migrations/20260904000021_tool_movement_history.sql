-- Полная история прибора: откуда был + снимок названий объектов
alter table public.tool_movements
  add column if not exists from_object_id uuid references public.objects(id),
  add column if not exists object_name text,
  add column if not exists from_object_name text;

-- Бэкап названий для уже существующих движений
update public.tool_movements m
...
declare
  ...
  v_from_object_id uuid;
  v_from_object_name text;
  v_to_object_name text;
begin
  select * into v_tool
  from public.tools
  ...
  -- Откуда был прибор до операции
  v_from_object_id := v_tool.current_object_id;
  if v_from_object_id is not null then
    select name into v_from_object_name from public.objects where id = v_from_object_id;
  end if;

  -- Куда / в контексте какого объекта произошло движение
  v_movement_object_id := case _movement_type
    when 'issue' then _object_id
    when 'extra_delivery' then _object_id
    when 'transfer' then _object_id
    when 'return' then coalesce(_object_id, v_tool.current_object_id)
    when 'to_repair' then coalesce(_object_id, v_tool.current_object_id)
    when 'loss' then coalesce(_object_id, v_tool.current_object_id)
    when 'write_off' then coalesce(_object_id, v_tool.current_object_id)
    else _object_id
  end;

  if v_movement_object_id is not null then
    select name into v_to_object_name from public.objects where id = v_movement_object_id;
  end if;

  perform set_config('kotov.skip_tool_guard', 'on', true);

  insert into public.tool_movements (
    tool_id, movement_type,
    object_id, object_name,
    from_object_id, from_object_name,
    from_holder_id, to_holder_id, moved_at, comment, created_by
  )
  values (
    _tool_id, _movement_type,
    v_movement_object_id, v_to_object_name,
    v_from_object_id, v_from_object_name,
